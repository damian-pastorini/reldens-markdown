...
class MarkdownToHTML
{
    ...
    constructor(config)
    {
        ...
        this.applyLazyOnImages = this.get(config, 'applyLazyOnImages', true);
    ...
    transform(markdown)
    {
        ...
        html = this.transformUrlElement(html, this.imageElement, (element, p1, p2, classAttr, url) => {
            let lazyAttr = this.applyLazyOnImages ? ' loading="lazy"' : '';
            return '<'+element.tag+' src="'+url+'" alt="'+p1+'"'+classAttr+lazyAttr+'/>';
        });
    ...
    extractCodeBlocks(text, codeBlocks)
    {
        return text.replace(/```([^\n]*)\n([\s\S]+?)```/g, (match, language, content) => {
            ...
            let codeBlock = '<pre><code'+languageClass+'>'+content+'</code></pre>';
    ...
    transformParagraphs(text)
    {
        ...
        for(let i = 0; i < lines.length; i++){
            ...
            if(!lines[i].startsWith('<') && !lines[i].startsWith('{{CODE_BLOCK_')){
                if(!inParagraph){
                    inParagraph = true;
